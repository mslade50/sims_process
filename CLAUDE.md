...
- **The sims' name contract is lowercase `'last, first'`** (e.g. `'scheffler, scottie'`) — dg_historical.db's `'Last, First'` just needs lowercasing + `name_replacements` (`_normalize_db_names()` in `sg_diagnostic.py`). Do NOT flip to `'first last'`; a well-meaning flip broke archetype matching entirely (2026-07). `clv.py` is the exception — DataGolf archive odds use first-last there. Also: `sg_diagnostic`'s db path honors the `DG_HISTORICAL_DB` env var and falls back to scanning `C:\Users\*\OneDrive` — the self-hosted runner runs as NetworkService whose `~` has no OneDrive (and no read ACL on user profiles either, so runtime db access fails on the runner regardless).
