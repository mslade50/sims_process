...
type OutrightsPayload = { edges: DataRow[]; fades: DataRow[]; finish: DataRow[]; probabilities: DataRow[] };

export function OutrightsView() {
  const { data, loading, error } = useDashboardData<OutrightsPayload>("outrights.json");
  const [tab, setTab] = useState<"edges" | "finish" | "probabilities">("edges");
  const [book, setBook] = useState("all");
  const [minEdge, setMinEdge] = useState(0);
  if (loading) return <LoadingState label="Loading live outright markets" />;
  if (error || !data) return <ErrorState message={error ?? "No outright snapshot was published."} />;

  const allRows = [...data.edges, ...data.fades, ...data.finish];
  const bookKey = (row: DataRow) => String(row.bookmaker ?? row.sportsbook ?? row.book ?? "").toLowerCase();
  const books = [...new Set(allRows.map(bookKey).filter(Boolean))].sort();
  const edgeValue = (row: DataRow) => numberValue(row.edge ?? row.edge_vs_devig ?? row.best_edge, 0);
  const filterRows = (rows: DataRow[]) => rows.filter((row) => (book === "all" || bookKey(row) === book) && edgeValue(row) >= minEdge);
  const edges = filterRows(data.edges).sort((a, b) => edgeValue(b) - edgeValue(a));
  const finish = filterRows(data.finish).sort((a, b) => edgeValue(b) - edgeValue(a));
  const bestEdge = Math.max(0, ...data.edges.map(edgeValue), ...data.finish.map(edgeValue));
  const playerCount = new Set([...data.edges, ...data.finish].map((row) => String(row.player_name ?? row.player ?? "")).filter(Boolean)).size;

  return (
    <>
      <PageIntro eyebrow="Live markets" title="Outrights" description="A cleaner view of live win and finish-position equity, with book-aware filtering and an instant probability board." controls={
        <div className="control-row">
          <SelectControl label="Sportsbook" value={book} onChange={setBook} options={[{ value: "all", label: "All books" }, ...books.map((value) => ({ value, label: titleCase(value) }))]} />
          <RangeControl label="Minimum edge" value={minEdge} min={0} max={25} onChange={setMinEdge} />
        </div>
      } />
      <div className="kpi-grid">
        <Kpi label="Best available edge" value={`${bestEdge.toFixed(1)}%`} detail="Across active markets" tone="positive" />
        <Kpi label="Positive positions" value={String(edges.length + finish.length)} detail={`At ${minEdge}%+ edge`} tone="accent" />
        <Kpi label="Players priced" value={String(playerCount)} detail="Live field coverage" />
        <Kpi label="Model markets" value={String(data.probabilities.length)} detail="Finish probability rows" />
      </div>
      <SegmentedControl label="Outright view" value={tab} onChange={setTab} options={[
        { value: "edges", label: "Win market" },
        { value: "finish", label: "Finish positions" },
        { value: "probabilities", label: "Probability board" },
      ]} />
      {tab === "edges" && (
        <div className="stack-lg">
          <Panel title="Positive win equity" eyebrow="Ranked by edge"><DataTable rows={edges} label="Win edges" preferredColumns={["player_name", "bookmaker", "market_odds", "my_fair", "sim_prob", "edge"]} /></Panel>
          <Panel title="Market fades" eyebrow="Model disagreement"><DataTable rows={data.fades} label="Fades" preferredColumns={["player_name", "bookmaker", "market_odds", "devig_prob", "sim_prob", "edge_vs_devig"]} /></Panel>
        </div>
      )}
      {tab === "finish" && <Panel title="Finish-position equity" eyebrow="Win · Top 5 · Top 10 · Top 20"><DataTable rows={finish} label="Finish positions" preferredColumns={["player_name", "market_type", "sportsbook", "american_odds", "my_fair", "sim_prob", "edge", "kelly_stake"]} /></Panel>}
      {tab === "probabilities" && <ProbabilityBoard rows={data.probabilities} />}
    </>
  );
}

function ProbabilityBoard({ rows }: { rows: DataRow[] }) {
  const sorted = [...rows].sort((a, b) => numberValue(b.simulated_win_prob) - numberValue(a.simulated_win_prob)).slice(0, 40);
  const columns = ["simulated_win_prob", "top_5", "top_10", "top_20"].filter((key) => sorted.some((row) => row[key] !== undefined));
  if (!sorted.length) return <EmptyState title="No probability board published" detail="Live finish probabilities will appear after the next simulation." />;
  return (
    <Panel title="Live finish probabilities" eyebrow="Top 40 by win probability">
      <div className="probability-board">
        <div className="probability-head"><span>Player</span>{columns.map((column) => <span key={column}>{titleCase(column.replace("simulated_", ""))}</span>)}</div>
        {sorted.map((row) => (
          <div className="probability-row" key={String(row.player_name)}>
            <strong>{titleCase(row.player_name)}</strong>
            {columns.map((column) => {
              const value = numberValue(row[column]);
              return <span key={column} style={{ "--heat": Math.min(1, value * (column === "simulated_win_prob" ? 8 : 1.8)) } as React.CSSProperties}>{(value * 100).toFixed(1)}%</span>;
            })}
          </div>
        ))}
      </div>
